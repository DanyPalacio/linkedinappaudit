...
/**
 * IMPORTANTE: Render (y plataformas similares) usan un gateway de salida que
 * cierra conexiones HTTP keep-alive inactivas después de un tiempo corto.
 * Si el SDK de Anthropic reutiliza una conexión que Render ya cerró del lado
 * del servidor sin avisar, Node lanza "Premature close" / "other side closed".
 * La solución es deshabilitar el keep-alive del agente HTTP usado para hablar
 * con la API de Anthropic, forzando una conexión TCP nueva en cada request.
 */
const anthropicHttpAgent = new https.Agent({ keepAlive: false });

// Initialize Anthropic client
const anthropic = new Anthropic({
  apiKey: process.env.ANTHROPIC_API_KEY,
  timeout: 120 * 1000, // 120s - evita que la conexión cuelgue indefinidamente
  maxRetries: 3,        // reintenta automáticamente ante fallos de red transitorios
  httpAgent: anthropicHttpAgent,
});
...
- "hashtagsRecommended": 10-15 hashtags específicos para ${profileData.industry} que maximizarán alcance en LinkedIn`;
  ...
  // Call Claude with vision - streaming evita "Premature close" en respuestas largas
...
Responde de manera concisa, profesional y accionable. Si la pregunta se relaciona con el análisis del perfil, usa el contexto provisto. Si es una pregunta general sobre LinkedIn o personal branding, responde con tu expertise.`;
    ...
    const chatResponse = await stream.finalMessage();
...
// Start server
app.listen(PORT, () => {
  console.log(`✅ Personal Branding Audit AI v4.1 running on port ${PORT}`);
  console.log(`🌐 Open http://localhost:${PORT}`);
  console.log(`📸 Image analysis (Claude Vision): ENABLED`);
  console.log(`🔌 HTTP keep-alive: DISABLED (avoids Premature close on Render)`);
});
